...
export class CategoriesService {
  ...
  protected readonly resourceUrl = `${serverApiUrl}${microserviceContextPath}catalog/api/categories`;
  protected readonly resourceSearchUrl = `${serverApiUrl}${microserviceContextPath}catalog/api/categories/_search`;
}
...
export class CategoryService extends CategoriesService {
  ...
  search(req: Search): Observable<ICategory[]> {
    const options = createRequestOption(req);
    return this.http.get<ICategory[]>(this.resourceSearchUrl, { params: options }).pipe(catchError(() => scheduled([], asapScheduler)));
  }
