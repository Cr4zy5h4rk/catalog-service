...
export default class Navbar implements OnInit {
  ...
  catalogEntityNavbarItems = signal<NavbarItem[] | null>(null);
  ...
  loadMicrofrontendsEntities(): void {
    ...
    loadNavbarItems('catalog').then(
      items => {
        this.catalogEntityNavbarItems.set(items);
      },
      (error: unknown) => {
        // eslint-disable-next-line no-console
        console.log('Error loading catalog entities', error);
      },
    );
  }
}
